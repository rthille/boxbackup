...
bool test_symlinks()
{
	...
	std::auto_ptr<IOStream> encoded(
		BackupStoreFile::EncodeFile("testfiles/testsymlink", 32, name));

	// Can't decode it from the stream, because it's in file order, and doesn't
	// have the required properties to be able to reorder it. So buffer it...
	...
	BackupStoreFile::DecodeFile(b, "testfiles/testsymlink_2",
		IOStream::TimeOutInfinite);
#endif
